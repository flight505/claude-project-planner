...
def extract_decisions_from_architecture(arch_content: str, blocks_content: str = "") -> list:
    """
    ...
    decision_patterns = [
        # Technology choices
        (r"(?:use|using|chose|selected|picked)\s+(PostgreSQL|MySQL|MongoDB|DynamoDB|Redis|Cassandra)", "database"),
        (r"(?:use|using|chose|selected)\s+(React|Vue|Angular|Next\.js|Svelte)", "frontend"),
        (r"(?:use|using|chose|selected)\s+(Node\.js|Python|Go|Rust|Java|\.NET)", "backend"),
        (r"(?:deploy(?:ed)?|host(?:ed)?)\s+(?:on|to)\s+(AWS|GCP|Azure|Vercel|Heroku)", "cloud"),

        # Architecture patterns
        (r"(microservices?|monolith(?:ic)?|serverless|event.driven|CQRS)", "architecture"),
...
def parse_model_response(response: str, model_name: str, decision: str) -> ValidationResult:
...
def calculate_consensus(results: list) -> tuple[str, str, dict]:
    ...
    """
    valid_results = [r for r in results if not r.error and r.recommendation != "unknown"]
    ...
    score_keys = ["scalability", "security_risk", "cost_effectiveness", "maintainability"]
    avg_scores = {}
...
def generate_report(
    ...
    model_names: list[str] = [
        MODEL_CONFIGS.get(m, {}).get("name") or m
...
def main():
    ...
    parser.add_argument("--building-blocks", type=Path,
                       help="Path to building blocks file")
    ...
    if not args.architecture_file.exists():
        print(f"Error: Architecture file not found: {args.architecture_file}", file=sys.stderr)
        sys.exit(1)
    ...
    if not valid_models:
        print(f"Error: No valid models specified. Available: {list(MODEL_CONFIGS.keys())}",
              file=sys.stderr)
        sys.exit(1)
    ...
    for i, decision in enumerate(decisions, 1):
        ...
        for model_key in valid_models:
            ...
            if error:
                ...
                result = parse_model_response(response, model_config["name"], decision["decision"])
                results.append(result)

        # Calculate consensus
        consensus, confidence, avg_scores = calculate_consensus(results)

        consensuses.append(DecisionConsensus(
            decision=decision["decision"],
